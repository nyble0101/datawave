...
public class ParallelIndexExpansion extends RebuildingVisitor {
    ...
    public boolean isExpandable(ASTERNode node) throws TableNotFoundException, JavaRegexAnalyzer.JavaRegexParseException {
        ...
        // if the identifier is not indexed, then we cannot expand it
        if (!helper.isIndexed(fieldName, config.getDatatypeFilter())) {
            return false;
        }
        ...
        // if the identifier starts with a wildcard and is not reverse indexed, then we cannot expand it
        if (analyzer.isLeadingRegex() && !helper.isReverseIndexed(fieldName, config.getDatatypeFilter())) {
            return false;
        }
     ...
     * @return
     */
    public boolean mustExpand(ASTERNode node) throws TableNotFoundException, JavaRegexAnalyzer.JavaRegexParseException {
        String fieldName = JexlASTHelper.getIdentifier(node);
